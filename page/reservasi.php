...
?>
            ...
            <div class="col-md-12">
              <div class="box box-info">
                <div class="box-header">
                  ...
                  <h3 class="box-title">Halaman Reservasi</h3>
                  <!-- tools box -->
                  ...
                  <div class="table-responsive">
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                        ...
                        //  order by a.tgl asc");
                        $tampil = $mysqli->query(" SELECT * FROM transaksi_pemesanan where reservasi = 'Reservation' order by tgl asc");
                        $no = 1;
                        while ($r = mysqli_fetch_array($tampil)) {
                          if ($r['status'] == 0) {
                            $status = "<button type='button' class='btn btn-danger'><i class ='fa  fa-exclamation-triangle'> Belum Konfirm</i></button>";
                          } else {
                            $status = "<button type='button' class='btn btn-primary' style='background-color: #dd3974 !important;''><i class ='fa fa-check-square'> Konfirm</i></button>";
                          }
                        ?>
                          <tr>
                            <td><?php echo "$no" ?></td>
                            ...
                            <td><?php echo "$r[status_reservasi]" ?></td>
                             <td><a href="?pg=reservasi&act=edit&id=<?php echo $r['kd_transaksi'] ?>"><button type="button" class="btn bg-orange"><i class="fa fa-pencil-square-o"></i></button></a></td>
                            <td><a href="?pg=reservasi&act=detail&id=<?php echo $r['id'] ?>" title="Detail Pemesanan"><button type="button" class="btn bg-orange"><i class="fa fa-eye"></i></button></a></td>
                            <td><a href="?pg=reservasi&act=delete_psn&id=<?php echo $r['id'] ?>" title="Hapus Pemesanan"><button type="button" class="btn btn-info" onclick="return confirm('Apakah anda yakin akan menghapusnya?');"><i class="fa fa-trash-o"></i></button></a></td>
                          </tr>
  ...
  VALUES ('','$_POST[kd_transaksi]','$tgl','$_POST[no_meja]','$_POST[atas_nama]','$total')");
    ...
    VALUES ('','$id_pemesanan', '$id_menu[$i]','$jumlah[$i]')");
      ...
      echo "<script>window.location='home.php?pg=reservasi&act=view'</script>";
    }
    ...
    VALUES ('','$_POST[tgl]','$_POST[id_menuku]','$_POST[id_harga]','$_POST[jumlah]','$total')");
      echo "<script>window.location='home.php?pg=reservasi&act=add'</script>";
    }
  ?>
      ...
      <div class="container">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><a href="?pg=reservasi&act=view">Data Transaksi Pemesanan</a></li>
                   ...
                   order by a.tgl asc");
                    ...
                    ?>
                      <tr>
                        ...
                        <td><a href="?pg=reservasi&act=delete&id=<?php echo $r['id_trans'] ?>"><button type="button" class="btn btn-info" onclick="return confirm('Apakah anda yakin akan menghapusnya?');"><i class="fa fa-trash-o"></i></button></a></td>
                      </tr>
  ...
  case 'delete':
    ...
    break;
  case 'edit':
  $d = mysqli_fetch_array($mysqli->query("SELECT * FROM transaksi_pemesanan WHERE kd_transaksi='$_GET[id]'"));
    if (isset($_POST['update'])) {
      $mysqli->query("UPDATE transaksi_pemesanan SET status_reservasi='$_POST[status_reservasi]', status='$_POST[status]'
  ...
  ?>  
    <div class="content-wrapper">
        <div class="container">
        <section class="content-header">
          <h1> Data Reservasi <?php echo $_GET['id'];?></h1>
        </section>
        <section class="content">
          <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="box-body">
                      <div class="form-group">
                        ...
                        <select class="form-control" id="status" name="status">
                          <option value="1" <?php if ($d['status'] == '1') { echo "selected"; } ?>>Sudah Bayar</option>
                          <option value="0" <?php if ($d['status'] == '0') { echo "selected"; } ?>>Belum Bayar</option>
                        </select>
                      </div>
                      <div class="form-group">
                        <label for="exampleInputEmail1">Konfirmasi Reservasi</label>                      
                        <select class="form-control" id="status_reservasi" name="status_reservasi">
                          <option value="" <?php if ($d['status_reservasi'] == '') { echo "selected"; } ?>>Belum Terlaksana</option>
                          <option value="Sudah" <?php if ($d['status_reservasi'] == 'Sudah') { echo "selected"; } ?>>Sudah Terlaksana</option>
                          <option value="Batal" <?php if ($d['status_reservasi'] == 'Batal') { echo "selected"; } ?>>Batal</option>
                        </select>
                      </div>
                      <div class="row">
                        <!-- left column -->
                        <div class="col-md-4 col-md-offset-5">
                          ...
                          &nbsp;
                          <button type="submit" onclick="history.go(-1)" class="btn btn-danger">Kembali</button>
                  </form>
                </div>
              </div><!-- /.box -->
            </div> <!-- /.col -->
